...
void Contour_Filter::depth_contour_filter(const cv::Mat& src)
{
  ...
  cv::Mat shadow_bin = shadow_filter(src_copy); //gets a binary image where the shadow contours of the objects are drawn
...
//      double area = contourArea(merged_contours[i]);
//      if (area < minArea || area > maxArea) //if area is not "lagom", ignore it
//        continue;

//      cv::RotatedRect rotatedRect = minAreaRect( cv::Mat(merged_contours[i]) );
...
//std::cout<<i+1<<" mc: "<<newObj.mc<<" contour area: "<<contourArea(newObj.contours_poly)<<std::endl;
      objects.push_back(newObj); //put the new object in the vector called objects
    }

  // Publish cropped RGB image
